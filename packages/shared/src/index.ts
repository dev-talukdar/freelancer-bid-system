...
export const DEFAULT_MAXIMUM_PROJECT_AGE_MINUTES = 10;
export const projectTypeSchema = z.enum(['fixed', 'hourly']);
export type ProjectType = z.infer<typeof projectTypeSchema>;
export const searchProfileSchema = z.object({
  name: z.string().min(1), enabled: z.boolean().default(true), keywords: z.array(z.string()).default([]), excludedKeywords: z.array(z.string()).default([]), jobIds: z.array(z.number().int().positive()).default([]), countries: z.array(z.string().min(2)).default([]), languages: z.array(z.string().min(2)).default([]), projectTypes: z.array(projectTypeSchema).default(['fixed','hourly']), minimumFixedBudget: z.number().nonnegative().nullable().optional(), maximumFixedBudget: z.number().nonnegative().nullable().optional(), minimumHourlyRate: z.number().nonnegative().nullable().optional(), maximumHourlyRate: z.number().nonnegative().nullable().optional(), maximumBidCount: z.number().int().nonnegative().nullable().optional(), pollIntervalSeconds: z.number().int().min(MIN_POLL_INTERVAL_SECONDS).default(DEFAULT_POLL_INTERVAL_SECONDS), notificationEnabled: z.boolean().default(true), soundEnabled: z.boolean().default(true), allowLocalProjects: z.boolean().default(false), maximumProjectAgeMinutes: z.number().int().min(1).max(1440).default(DEFAULT_MAXIMUM_PROJECT_AGE_MINUTES),
});
