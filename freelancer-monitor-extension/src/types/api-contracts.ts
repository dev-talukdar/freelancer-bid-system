import { z } from 'zod';
/**
 * Mirrors the API response contract.
 * Update this type when the API response shape changes.
 */
...
export const searchProfileSchema = z.object({
  ...
  allowLocalProjects: z.boolean().default(false),
  maximumProjectAgeMinutes: z
    .number()
    .int()
    .min(1)
    .max(1440)
...
export interface HealthDto {
  ...
  monitoring: MonitorStatusDto;
  freelancerTokenConfigured: boolean;
  freelancerTokenExpiresAt?: string;
  freelancerTokenExpirationWarning: boolean;
}
export interface PaginatedDetectedProjectsDto {
...
export function normalizeFreelancerSeoUrl(input: string): string {
  const cleaned = input
    .trim()
    .replace(/^https?:\/\/(www\.)?freelancer\.com\/projects\//i, '')
    .replace(/^\/+|\/+$/g, '');
  if (!/^[A-Za-z0-9/_-]+$/.test(cleaned) || cleaned.includes('..'))
    throw new Error('Invalid Freelancer project path');
  return cleaned;
}
export function toFreelancerProjectUrl(seoUrl: string): string {
  return `${FREELANCER_PROJECT_BASE_URL}${normalizeFreelancerSeoUrl(seoUrl)}`;
}
