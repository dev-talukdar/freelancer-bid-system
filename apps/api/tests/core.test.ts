...
describe('mongoose payload builders', () => {
  ...
  it('fills missing budget defaults without aggregation-pipeline updates at startup', async () => {
    const updateMany = vi
      .spyOn(SearchProfileModel, 'updateMany')
      .mockResolvedValue({ modifiedCount: 1 } as never);

    await expect(enforceActiveProfileBudgetDefaults()).resolves.toBe(true);

    expect(updateMany).toHaveBeenCalledTimes(4);
    for (const call of updateMany.mock.calls) {
      expect(Array.isArray(call[1])).toBe(false);
    }
    expect(updateMany.mock.calls[0]?.[1]).toEqual({ $set: { minimumFixedBudget: 250 } });
    expect(updateMany.mock.calls[1]?.[1]).toEqual({ $set: { maximumFixedBudget: 50000 } });
    expect(updateMany.mock.calls[2]?.[1]).toEqual({ $set: { minimumHourlyRate: 15 } });
    expect(updateMany.mock.calls[3]?.[1]).toEqual({ $set: { maximumHourlyRate: 100 } });
