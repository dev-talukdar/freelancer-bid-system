...
export const enforceActiveProfileBudgetDefaults = async (): Promise<boolean> => {
  // Use plain update documents instead of an aggregation-pipeline update so startup remains
  // compatible with older/self-hosted MongoDB variants. Each update only fills its own
  // missing/null field and leaves any existing custom budget values untouched.
  const results = await Promise.all([
    SearchProfileModel.updateMany(
      { $or: [{ minimumFixedBudget: null }, { minimumFixedBudget: { $exists: false } }] },
      { $set: { minimumFixedBudget: requiredBudgetDefaults.minimumFixedBudget } },
    ),
    SearchProfileModel.updateMany(
      { $or: [{ maximumFixedBudget: null }, { maximumFixedBudget: { $exists: false } }] },
      { $set: { maximumFixedBudget: requiredBudgetDefaults.maximumFixedBudget } },
    ),
    SearchProfileModel.updateMany(
      { $or: [{ minimumHourlyRate: null }, { minimumHourlyRate: { $exists: false } }] },
      { $set: { minimumHourlyRate: requiredBudgetDefaults.minimumHourlyRate } },
    ),
    SearchProfileModel.updateMany(
      { $or: [{ maximumHourlyRate: null }, { maximumHourlyRate: { $exists: false } }] },
      { $set: { maximumHourlyRate: requiredBudgetDefaults.maximumHourlyRate } },
    ),
  ]);
