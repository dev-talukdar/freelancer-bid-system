...
import { buildPopupViewModel, getApiKeyStatus, trimSecret } from './view-model.js';
...
describe('popup view model', () => {
  it('shows running state and makes start unavailable while already running', () => {
    const vm = buildPopupViewModel({ health: health(true), secret: 'secret', isPolling: false, actionPending: false });
  ...
  it('shows stopped state and makes stop unavailable while already stopped', () => {
    const vm = buildPopupViewModel({ health: health(false), secret: 'secret', isPolling: false, actionPending: false });
  ...
  it('disables Poll now while polling', () => {
    const vm = buildPopupViewModel({ health: health(false), secret: 'secret', isPolling: true, actionPending: false });
  ...
  it('handles backend disconnected state without showing monitoring healthy', () => {
    ...
    const vm = buildPopupViewModel({ health: disconnected, secret: 'secret', isPolling: false, actionPending: false });
